...
class _HomeScreenState extends State<HomeScreen> {
  ...
  getHomePageGraphs(
    ...
    final res = await Provider.of<HomeProvider>(
      context,
      listen: false,
    ).getHomePageGraphs(reqBody, 
      Provider.of<AuthProvider>(context, listen: false).user.id,
    );
    setState(() {
      priceTrend = Provider.of<HomeProvider>(context, listen: false).priceTrend;
      topDistricts = Provider.of<HomeProvider>(context,
        listen: false,
      ).topDistricts;
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              Container(
                child: Text("Haa bhai aarha hai...", style: GoogleFonts.poppins(
                  textStyle: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.w500,
                    color: Colors.black,
...
class _TableRowState extends State<TableRow> {
  ...
  pinMandi()async{
    final reqBody = {
      "mandi_name": widget.mandiName,
    };

    final res = await Provider.of<PinnedMandiProvider>(context, listen: false).pinMandi(reqBody);
    // if(res["success"]){
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: MouseRegion(
        ...
        child: Container(
          decoration: BoxDecoration(
            color: !widget.isHeader && _isHovered ? primaryYellowLight : widget.bgColor,
            border: Border(
              bottom: BorderSide(color: Colors.grey.shade300, width: 1),
              
            ),
            borderRadius: widget.isHeader ? BorderRadius.only(topLeft: Radius.circular(10), topRight: Radius.circular(10)) : null
          ),
              ...
              Expanded(
                child: Container(
                  ...
                  child: Text(
                    ...
                    style: GoogleFonts.poppins(
                      textStyle: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color:widget.isHeader ? Colors.white: Colors.black),
                    ),
                  ),
                ),
              ),
...
class PinnedMandiCard extends StatelessWidget {
  const PinnedMandiCard({super.key});
...
class _HomeTopBarState extends State<HomeTopBar> {
  ...
  getTab({required String text, required IconData icon, required Function onClick, required int index}){
    return MouseRegion(
      cursor: SystemMouseCursors.click,
            onEnter: (event){
              setState(() {
            ...
            child: GestureDetector(
              onTap: 
                (){
                  Provider.of<HomeProvider>(context, listen: false).setSelectedPage(index);
                },
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: Provider.of<HomeProvider>(context).selectedPage == index ? Colors.black : hoveredTab == index ? Colors.grey.shade500 : Colors.white,
                  borderRadius: BorderRadius.circular(100),
                ),
                child: Center(
                  child: Row(
                    children: [
                      Icon(icon, color: (hoveredTab == index)|| (Provider.of<HomeProvider>(context).selectedPage == index)  ? Colors.white : Colors.black,),
                      Padding(
                        padding: const EdgeInsets.only(left: 5),
                        child: Text(text, style: GoogleFonts.poppins(
                          textStyle: TextStyle(
                            color: (hoveredTab == index) ||
                                (Provider.of<HomeProvider>(
                                      context,
                                    ).selectedPage ==
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          getTab(text: "Home", icon: Icons.home_filled, index: 1, onClick: (){}),
          getTab(text: "Plant Disease Prediction", icon: Icons.settings, index: 2, onClick: (){}),
          getTab(text: "Dashboard", icon: Icons.dashboard_rounded, index: 3, onClick: (){}),
